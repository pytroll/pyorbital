...
class Moon(object):
    def __init__(self, dtobj):
        ...
        lat = np.rad2deg(np.arctan2(zg_ , np.sqrt(xg_ * xg_ + yg_ * yg_))) + self.dlat
    ...
    def topocentric_position(self, longitude, latitude):
        ...
        topDecl = (self.declination - 
                   ...
                   np.sin(np.deg2rad(g__ - self.declination)) / np.sin(np.deg2rad(g__))
                   )
    ...
    def pertubations(self):
        ...
        dlon = (-1.274 * np.sin(np.deg2rad(Mm_ - 2*D__))    # (the Evection)
                 ...
                 -0.186 * np.sin(np.deg2rad(Ms_))           # (the Yearly Equation)
                 ...
                 -0.035 * np.sin(np.deg2rad(D__))           # (the Parallactic Equation)
...
def azimuthal_coord(utc_time, lonsun, local_lon, local_lat, 
    ...
    alt = np.rad2deg(np.arcsin( zhor )) # = atan2( zhor, sqrt(xhor*xhor+yhor*yhor) )
...
def ecliptic_to_equatorial(xg_, yg_, zg_, ecl):
    ...
    rg_ = np.sqrt(xg_ * xg_ + yg_ * yg_ + zg_ * zg_) # = sqrt(xe*xe+ye*ye+ze*ze)
